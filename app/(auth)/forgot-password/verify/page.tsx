...
export default function PaswordVerificationPage() {
  const {  email } =
  ...
  const { mutateAsync, isPending } = useMutate<Response, Payload>({
    type: "post",
    url: "auth/password-reset/confirm-otp/",
  });
    ...
    try {
      ...
      const response = await mutateAsync({
        ...
        if (response?.data?.data) {
            ...
            router.push(
              `/forgot-password/reset?userId=${response?.data?.data?.user_id}&token=${response?.data?.data?.token}`
            );

...
export default function VerificationPage() {
  const { setUser, setToken, email, setEmail, setRefreshToken } =
      ...
      <div className="h-full flex items-center justify-center bg-white lg:w-1/2">
        <div className="w-full max-w-[480px] p-6 md:p-8">
          ...
          <Button
            ...
            isLoading={isPending}
            className="w-full h-[52px]  text-white text-sm font-medium rounded-lg"
          >
            Submit
          </Button>
        </div>
      ...
      <Dialog open={showSuccess} onOpenChange={setShowSuccess}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader className="text-center space-y-4">
            ...
            <DialogDescription>Verification successful</DialogDescription>
            <div className="flex justify-center py-4">
              <span className="text-4xl text-center">🎉</span>
            </div>
            <p className="font-medium text-center">Welcome to Livio Care</p>
          </DialogHeader>
          <Button
            onClick={() => {
              setEmail("");
